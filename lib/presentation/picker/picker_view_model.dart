...
class PickerViewModel extends GetxController {
  ...
  _getPixelImage() async {
    ...
    imageSizeString = 'width: ${width}px, height: ${height}px';
    Log.i('image width: $imageSizeString');

    pixelWidthCount = 1000; // int.parse(pixelInputController.text);
    ...
    for (int y = 1; y < pixelHeightCount + 1; y++) {
      for (int x = 1; x < pixelWidthCount + 1; x++) {
        int pixel = image.getPixel(x * xChunk, y * yChunk);
